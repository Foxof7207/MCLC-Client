    ...
    try {
        ...
        if (data && data.loggedIn) {
            ...
            const uploadBtn = isExtensionsPage
                ? `<a href="dashboard.html" class="hidden md:block bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all">Upload</a>`
                : '';

            const adminBtn = data.user.role === 'admin'
